...
const getCollection = async () => {
	...
	collection = client.db(dbName).collection("weather_events");
	await ensureIndexes();
	logger.info(`MongoDB connected (${dbName}.weather_events)`, "db.weather");
	return collection;
};

exports.isDatabaseEnabled = isDatabaseEnabled;

exports.connectWeatherRepository = async () => {
	try {
		await getCollection();
	} catch (err) {
		logger.error(`MongoDB connection failed: ${err.message}`, "db.weather");
		throw err;
	}
};

exports.closeWeatherRepository = async () => {
	if (!client) return;
	await client.close();
	client = null;
	collection = null;
...
exports.upsertWeatherEvents = async (events = []) => {
